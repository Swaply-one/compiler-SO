...
import Page_HomeScreen from "./pages/Page_HomeScreen";
...
export default function App() {
  ...
  const [activePage, setActivePage] = useState("home"); // Default to the brand new Hacker Home Screen!
  ...
  const lastActiveRouteRef = useRef("home");
  ...
  useEffect(() => {
    const handleUrlCheck = () => {
      ...
      if (path.includes("home") || search.includes("home") || hash.includes("home")) {
        setActivePage("home");
      } else if (path.includes("404") || search.includes("404") || hash.includes("404")) {
  ...
  const restorePreviousRoute = () => {
    ...
    setActivePage(lastActiveRouteRef.current || "home");
  };
  ...
  const renderActivePage = () => {
    ...
    switch (activePage) {
      case "home":
        return (
          <Page_HomeScreen
            onNavigateTerminal={() => setActivePage("retro_terminal")}
            onNavigateNetwork={() => setActivePage("page_network")}
            onNavigate404={() => setActivePage("page_404")}
            onNavigateLaptop={() => setActivePage("page_server_laptop")}
            onNavigateOTP={() => setActivePage("otp_verification")}
          />
        );
      ...
      default:
        return (
          <Page_RetroCRTTerminal
            ...
            onNavigate404={() => setActivePage("page_404")}
            onOpenOTP={() => setActivePage("otp_verification")}
          />
        );
    }
  };
  ...
  const navButtons = [
    { id: "home", label: "🏠 Hacker Home", color: "#39ff14" },
  ...
  return (
    <div style={{ minHeight: "100vh", backgroundColor: "#050805", color: "#f8fafc" }}>
      {/* Floating Quick Navigation Switcher (Hidden on Home Screen to avoid overlapping compiler controls) */}
      {activePage !== "home" && (
        <div
          style={{
            position: "fixed",
